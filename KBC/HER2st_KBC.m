clear
clc
maindir = 'DataSet/HER2st';
subdir  = dir( maindir );
resultmean = strings(1,1);
resulttop =  strings(1,1);
parameterset = zeros(1,1);

mat_data = load('../Dataset/HER2st/wl/sample_A1.mat');
class = mat_data.class;
class = double(class);
data = mat_data.data;
pos = mat_data.pos;
...
%% parameter setting
%--------------------------------------------------------------------------
psilist=[4 6 8 16 24 32 48 64 80 100 200 250 512];
...
resultmean(1,runtimes) = num2str(bestARI);
parameterset(1,1) = index2
bestLabel = refineMethod(pos,bestLabel,"hexagon");
[ARI,RI,MI,HI]=RandIndex(class,bestLabel);
resulttop(1,runtimes)  = num2str(ARI);
